...
// Triad: RPD port from funky-station (PR #1338). Cursor-quadrant inside the tile picks the AtmosPipeLayer for the
// placement; mouse near tile center → Primary, NE quadrant → Secondary, SW quadrant → Tertiary. The picked layer is
// applied to the placement ghost via TryGetAlternativePrototype; the same math runs server-side from
// LastKnownEyeRotation when the player commits the placement.
